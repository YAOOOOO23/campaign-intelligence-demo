...
export default function Home() {
  ...
  const renderCalendarTable = (section: { label: string; start: string; end: string }) => {
    ...
    return <div key={section.label} className="overflow-hidden border border-black/10 bg-white"><div className="flex items-center justify-between border-b border-black/10 bg-[#181716] px-3 py-2.5 text-white"><span className="text-[10px] font-semibold uppercase tracking-wider text-white/50 sm:text-xs">Campaign · {section.label}</span><span className="text-[9px] text-white/45">{sectionItems.length} 项</span></div><div className="grid border-b border-black/10 bg-[#292827] text-white" style={{ gridTemplateColumns: "minmax(108px, 1.25fr) minmax(0, 8.75fr)" }}><div className="px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-white/50">Campaign</div><div className="flex">{sectionMonths.map((month) => <div key={month.label} className="border-l border-white/10 px-0.5 py-2 text-center text-[10px] font-semibold sm:text-xs" style={{ flexGrow: month.days, flexBasis: 0 }}>{month.label}</div>)}</div></div><div className="divide-y divide-black/10">{sectionItems.map((item, index) => {
      const visiblePeriods = calendarPeriodsFor(item).filter((p) => validDate(p.start) && (p.end || p.start) >= section.start && p.start <= section.end);
      const trackHeight = Math.max(54, visiblePeriods.length * 30 + 14);
      return <button key={item.id} onClick={() => setSelected(item)} className="grid w-full text-left transition hover:bg-[#fff7f7]" style={{ gridTemplateColumns: "minmax(108px, 1.25fr) minmax(0, 8.75fr)" }}><div className="min-w-0 px-3 py-3"><p className="truncate text-xs font-semibold sm:text-sm">{item.name}</p><p className="mt-1 text-[9px] text-black/45 sm:text-[10px]">{item.type} · {visiblePeriods.length}个已核验节点</p></div><div className="relative min-w-0 border-l border-black/10" style={{ minHeight: trackHeight }}>{sectionMonths.slice(1).map((month) => <div key={month.label} className="absolute inset-y-0 border-l border-black/8" style={{ left: `${(month.startOffset / sectionTotalDays) * 100}%` }} />)}{visiblePeriods.map((p, periodIndex) => { const sourceStart = p.start as string; const sourceEnd = validDate(p.end) ? p.end : sourceStart; const start = dateAtOrAfter(sourceStart, section.start); const end = dateAtOrBefore(sourceEnd, section.end); const left = (daysBetween(start, section.start) / sectionTotalDays) * 100; const width = ((daysBetween(end, start) + 1) / sectionTotalDays) * 100; const color = index % 2 ? calendarSecondary : calendarPrimary; const sameDay = start === end; const sameMonth = start.slice(0, 7) === end.slice(0, 7); const displayLabel = sameDay ? `${start.slice(5, 7)}/${start.slice(8, 10)}` : sameMonth ? `${start.slice(5, 7)}/${start.slice(8, 10)}\n-${end.slice(8, 10)}` : `${start.slice(5, 7)}/${start.slice(8, 10)}\n${end.slice(5, 7)}/${end.slice(8, 10)}`; const fullLabel = sourceStart === sourceEnd ? formatDate(sourceStart) : `${formatDate(sourceStart)}—${formatDate(sourceEnd)}`; const labelMinWidth = sameDay ? 34 : sameMonth ? 52 : 64; const alignRight = left > 94; const edgePosition = alignRight ? { right: `${(daysBetween(section.end, end) / sectionTotalDays) * 100}%` } : { left: `${left}%` }; return <div key={`${sourceStart}-${periodIndex}`} style={{ ...edgePosition, width: `${width}%`, minWidth: labelMinWidth, top: 9 + periodIndex * 30, backgroundColor: color }} className="absolute z-10 flex h-[22px] items-center px-1 text-white shadow-sm" title={`${p.label}：${fullLabel}`}><span className="w-full whitespace-pre-line text-center text-[8px] font-bold leading-[9px] sm:text-[9px] sm:leading-[10px]">{displayLabel}</span></div>; })}</div></button>;
